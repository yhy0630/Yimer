...
export default {
	...
	computed: {
		...
		strictTreasureShopItems() {
			return this.strictGoodsList.map(item => ({
				id: item.id,
				image: item.image,
				shopName: this.currentCampus.name || '伊美尔推荐',
				title: item.title || item.name,
				authorAvatar: item.authorAvatar || '/static/linshi/01.png',
				authorName: item.authorName || (item.tags && item.tags[0]) || '伊美尔推荐官',
				likes: item.salesSum || 0
			}))
		}
	},
	methods: {
		...
		handleStrictTreasureSelect(item) {
